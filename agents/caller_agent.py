...
# role: module
# neighbors: __init__.py, custom_command_generator.py, security_classifier.py
# exports: CallerAgent
...
# Markdown patterns that break TTS
_MARKDOWN_PATTERNS = [
...
def _clean_for_tts(text: str) -> str:
    """
    Remove markdown formatting that would be spoken literally by TTS.
    Asterisks, underscores, backticks have no place in spoken audio.
    """
...
class CallerAgent:
    ...
    def add_bank_response(self, content: str, agent_label: str) -> None:
        """Bank response → stored as 'assistant'. Merge if consecutive."""
    ...
    def add_own_turn(self, content: str) -> None:
        """Caller speech → stored as 'user'. Merge if consecutive."""
    ...
    async def speak(self, turn_config: TurnConfig) -> str:
        ...
        for i in range(1, len(non_sys)):
            if non_sys[i]["role"] == non_sys[i - 1]["role"]:
                logger.error("Role alternation violation at %d: %s→%s", i,
                             non_sys[i-1]["role"], non_sys[i]["role"])
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                NEBIUS_API_URL, headers=headers, json=payload,
