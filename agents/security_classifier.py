...
Switched from MiniMax (which emits <think> blocks that overflow token budgets)
to Gemma (clean output, no chain-of-thought, fast responses).
"""
...
class SecurityClassifier:
    ...
    async def classify(
        ...
        payload = {
            ...
            "messages": messages,
            "temperature": 0.1,
            "max_tokens": 20,  # Gemma gives clean one-word answers
        }
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    # Take first word, uppercase, strip punctuation
                    first_word = raw.upper().replace(".", "").replace("'", "").split()[0] if raw.split() else ""
                    try:
