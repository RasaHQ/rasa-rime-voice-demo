...
# role: module
# neighbors: __init__.py, asr_service.py, tts_service.py
# exports: DemoLogger
...
event during the demo: LLM calls, TTS requests, ASR results, Rasa
exchanges, security classifications, and errors.

Usage:
    from services.demo_logger import DemoLogger
    log = DemoLogger()
    log.llm_request("caller", messages, response_text)
    log.tts_request("rasa", text, num_bytes)
    log.rasa_exchange(user_text, bot_response)
...
class DemoLogger:
    ...
    def llm_request(
        ...
        # Truncate messages for readability in TXT
        msg_summary = [
            {"role": m["role"], "content": m["content"][:120] + ("…" if len(m["content"]) > 120 else "")}
            for m in messages[-3:]  # last 3 messages only in TXT
        ]
        ...
        self._write_txt(f"  → response: {response[:300]}" + ("…" if len(response) > 300 else ""))
    ...
    def tts_request(
        ...
        """Log a Rime TTS request."""
        self._write_txt(f"\n[TTS → RIME]  agent={agent_role}  voice={voice}")
        self._write_txt(f"  text: {text[:200]}" + ("…" if len(text) > 200 else ""))
        self._write_txt(f"  → {audio_bytes:,} bytes audio" + (f"  ({duration_ms:.0f}ms)" if duration_ms else ""))
    ...
    def asr_request(
        ...
        """Log a Deepgram ASR request."""
        self._write_txt(f"\n[ASR → DEEPGRAM]  file={audio_file}")
        self._write_txt(f"  → transcript: {transcript!r}" + (f"  ({duration_ms:.0f}ms)" if duration_ms else ""))
    ...
    def rasa_exchange(
        ...
        """Log a Rasa request/response pair."""
        self._write_txt(f"\n[RASA]  sender={sender_id}")
        self._write_txt(f"  user:  {user_message[:200]}")
        self._write_txt(f"  → bot: {bot_response[:200]}" + ("…" if len(bot_response) > 200 else "") + (f"  ({duration_ms:.0f}ms)" if duration_ms else ""))
        self._emit("rasa_exchange", {
            "sender_id": sender_id,
    ...
    def transfer_event(self, prior_turn: int, rasa_response: str) -> None:
        """Log the moment Rasa transfers to the LLM manager."""
        self._txt_separator(f"⚠  TRANSFER TO LLM MANAGER  (after turn {prior_turn})")
        self._write_txt(f"  Rasa said: {rasa_response}")
        self._emit("transfer_event", {
            "prior_turn": prior_turn,
            "rasa_response": rasa_response,
        })

    def error(self, component: str, description: str, exc: Optional[Exception] = None) -> None:
